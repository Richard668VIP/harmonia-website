...
export async function POST(req: NextRequest) {
  try {
    ...
    await resend.emails.send({ from:"Harmonia <hello@harmoniawisdom.com>", to:["hello@harmoniawisdom.com"], reply_to:email, subject:`[${type}] Enquiry from ${name}`, html:`<h2>${name}</h2><p>${email}</p><p>${message}</p>` });
